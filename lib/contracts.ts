...
export function parsePublishRequest(payload: unknown): NormalizedPublishRequest {
  if (!isRecord(payload)) {
  ...
  const baseBranch = expectString(registryValue.baseBranch, 'registry.baseBranch');
  ...
  const files: PublishFilePayload[] = filesValue.map((entry, index) => {
    ...
    const entryPath = normalizeRelativePath(expectString(entry.path, `skill.files[${index}].path`));
    const encoding = expectString(entry.encoding, `skill.files[${index}].encoding`);
    if (encoding !== 'base64') {
      throw new Error(`skill.files[${index}].encoding must be 'base64'.`);
    }

    const content = expectString(entry.content, `skill.files[${index}].content`);
